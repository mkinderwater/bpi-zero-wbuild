#!/usr/bin/env bash
# Fixed release helper: build MAX98357A only for Debian 13 armhf 6.12.107+deb13-armmp.
# This is intentionally not a generalized/dynamic kernel builder.
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ABI="6.12.107+deb13-armmp"
DEBIAN_VERSION="6.12.107-1"
COMMON_NAME="linux-headers-6.12.107+deb13-common"
KBUILD_NAME="linux-kbuild-6.12.107+deb13"
SEC_BASE="https://deb.debian.org/debian-security/pool/updates/main/l/linux"
: "${DEBIAN_SNAPSHOT_STAMP:=20260907T235959Z}"
SNAP_BASE="https://snapshot.debian.org/archive/debian-security/${DEBIAN_SNAPSHOT_STAMP}/pool/updates/main/l/linux"
WORK_DIR="${MK_MAX98357A_BUILD_WORK:-${WORK_DIR:-$HERE/build}/max98357a-source}"
OUT_DIR="${MK_MAX98357A_BUILD_OUT:-$HERE/build/max98357a-module}"
SOURCE="$HERE/hardware/max98357a/max98357a.c"
...
fetch() {
    ...
    rm -f "$out.tmp"
    echo ">> primary unavailable; trying snapshot $fallback"
    if curl -fL --retry 3 --retry-delay 1 -o "$out.tmp" "$fallback"; then
        mv "$out.tmp" "$out"
        return 0
    fi
    rm -f "$out.tmp"
    echo "ERROR: required fixed-kernel build package unavailable." >&2
    echo "       primary:  $url" >&2
    echo "       snapshot: $fallback" >&2
    return 1
}
...
fetch "$SEC_BASE/linux-headers-${ABI}_${DEBIAN_VERSION}_armhf.deb" "$SNAP_BASE/linux-headers-${ABI}_${DEBIAN_VERSION}_armhf.deb" "$HDR_ARM"
fetch "$SEC_BASE/${COMMON_NAME}_${DEBIAN_VERSION}_all.deb" "$SNAP_BASE/${COMMON_NAME}_${DEBIAN_VERSION}_all.deb" "$HDR_COMMON"
fetch "$SEC_BASE/${KBUILD_NAME}_${DEBIAN_VERSION}_${KBUILD_ARCH}.deb" "$SNAP_BASE/${KBUILD_NAME}_${DEBIAN_VERSION}_${KBUILD_ARCH}.deb" "$KBUILD"
...
cp "$HERE/hardware/max98357a/Makefile" "$WORK_DIR/module/Makefile"

export ARCH=arm
export CROSS_COMPILE="$CROSS_COMPILE_PREFIX"
export KBUILD_BUILD_USER=icubedev
export KBUILD_BUILD_HOST=bpi-m2-zero
